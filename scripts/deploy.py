...
def deploy_token_farm_and_dapp_token():
    ...
    dict_of_allowed_tokens = {
        dapp_token: get_contract('fau_usd_price_feed'),
        fau_token: get_contract('fau_usd_price_feed'),
...
def add_allowed_tokens(token_farm, dict_of_allowed_tokens, account):
    for token in dict_of_allowed_tokens:
        ...
        tx = token_farm.setPriceFeedContract(token.address, dict_of_allowed_tokens[token], {'from': account})
        tx.wait(1)
